...
export type RepairJob = {
  ...
  // Same scan, for the "Authorised Signature" (branch/PIC) line instead
  // of the customer's — independent of signatureStatus, since a job can
  // be missing either one on its own.
  picSignatureStatus: string;
  // Manually cleared by Management after checking the actual jobsheet
  // photo — a "not_detected" job (either signature) stays flagged in the
  // Errors tab until this is set, regardless of how many times the job
  // itself is edited.
