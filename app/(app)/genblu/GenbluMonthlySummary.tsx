        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-neutral-50 text-left text-xs font-medium text-neutral-500 uppercase tracking-wide">
              ...
              <th className="px-4 py-2.5 text-right">Counts</th>
          ...
          <tbody>
            {summary.rows.map((r) => (
              <tr key={r.branch} className="border-t border-neutral-100">
                ...
                <td className="px-4 py-2.5 text-right text-neutral-700">{r.counts}</td>
                <td className="px-4 py-2.5 text-right text-neutral-700">{r.points.toLocaleString()}</td>
              </tr>
            ))}
            <tr className="border-t border-neutral-200 bg-neutral-50 font-semibold text-neutral-900">
              <td className="px-4 py-2.5">Total</td>
              <td className="px-4 py-2.5 text-right">{summary.total.counts}</td>
              <td className="px-4 py-2.5 text-right">{summary.total.points.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
